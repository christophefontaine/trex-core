...
#include "trex_driver_lio.h"
#include "trex_driver_defines.h"

CTRexExtendedDriverBaseLio::CTRexExtendedDriverBaseLio() {
...
int CTRexExtendedDriverBaseLio::get_min_sample_rate(void){
    return (RX_CHECK_MIX_SAMPLE_RATE);
}

void CTRexExtendedDriverBaseLio::get_dpdk_drv_params(CTrexDpdkParams &p) {
...
void CTRexExtendedDriverBaseLio::clear_extended_stats(CPhyEthIF * _if){
    repid_t repid=_if->get_repid();
    rte_eth_stats_reset(repid);
}

void CTRexExtendedDriverBaseLio::update_configuration(port_cfg_t * cfg){
...
void CTRexExtendedDriverBaseLio::reset_rx_stats(CPhyEthIF * _if, uint32_t *stats, int min, int len) {
    for (int i =0; i < len; i++) {
        stats[i] = 0;
    }
}

int CTRexExtendedDriverBaseLio::get_rx_stats(CPhyEthIF * _if, uint32_t *pkts, uint32_t *prev_pkts
                                             ,uint32_t *bytes, uint32_t *prev_bytes, int min, int max) {
    /* not supported yet */
    return 0;
}

int CTRexExtendedDriverBaseLio::dump_fdir_global_stats(CPhyEthIF * _if, FILE *fd)
{
    repid_t repid=_if->get_repid();
...
bool CTRexExtendedDriverBaseLio::get_extended_stats(CPhyEthIF * _if, CPhyEthIFStats *stats) {
...
int CTRexExtendedDriverBaseLio::wait_for_stable_link(){
    delay(20);
    return (0);
}

CFlowStatParser *CTRexExtendedDriverBaseLio::get_flow_stat_parser() {
    CFlowStatParser *parser = new CFlowStatParser(CFlowStatParser::FLOW_STAT_PARSER_MODE_HW);
    assert (parser);
    return parser;
}

void CTRexExtendedDriverBaseLio::get_rx_stat_capabilities(uint16_t &flags, uint16_t &num_counters, uint16_t &base_ip_id) {
